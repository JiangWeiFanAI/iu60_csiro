...
#     criterion=nn.MSELoss()
    ...
    scheduler = optim.lr_scheduler.ExponentialLR(optimizer_my, gamma=0.99)
    epoch=0
    if pre_train_path!=".":
...
#         net.load(pre_train_path)
        ...
        scheduler = optim.lr_scheduler.ExponentialLR(optimizer_my, gamma=0.99,last_epoch=epoch)
...
#                 break                
        ...
        write_log("epoche: %d,time cost %f s, lr: %f, train_loss: %f,validation loss:%f,psnr:%f "%(
                  e,
                  time.time()-start,
                  optimizer_my.state_dict()['param_groups'][0]['lr'],
                  loss/len(train_data),
                  total_val_loss/len(val_dataloders),
                  eval_psnr/len(val_dataloders)  
             ))
...
#                   val_loss
#              ))
        if total_val_loss<val_max_error:
            val_max_error=total_val_loss
    #         torch.save(net,train_loss"_"+str(e)+".pkl")
            if not os.path.exists("./model/save/"+args.train_name+"/"):
                os.mkdir("./model/save/"+args.train_name+"/")
            write_log("saving")
            state = {'model': net.state_dict(), 'optimizer': optimizer_my.state_dict(), 'epoch': e}
            torch.save(state, "./model/save/"+args.train_name+"/"+"best_test.pth")
        if loss<max_error:
            max_error=loss
    #         torch.save(net,train_loss"_"+str(e)+".pkl")
            if not os.path.exists("./model/save/"+args.train_name+"/"):
            ...
            write_log("saving")
            state = {'model': net.state_dict(), 'optimizer': optimizer_my.state_dict(), 'epoch': e}
            torch.save(state, "./model/save/"+args.train_name+"/best_train.pth")
